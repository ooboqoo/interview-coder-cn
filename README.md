...
- **截屏解题**：支持全屏与双点区域截图（可附带电脑声音的实时转录文字），发送给视觉大模型分析并流式展示解答；追加截图可先进入待发送队列，检查无误后再一次性发送
...
普通截图会立即发送给 AI，并新开一轮对话。追加截图和区域截图只会进入待发送队列；区域截图启动后，依次点击目标区域的左上角和右下角即可完成截取。确认队列内容后，使用“发送待发送截图”快捷键一次性提交全部截图。也可以撤回上一张或清空队列，这两项操作都不会影响已经发送的截图、回答或对话历史。所有快捷键均可在设置中修改。
